...
def speak_reminder(reminder_id):
    ...
    if not reminder:
        return
    print(
        f"[Reminder Triggered] Time: {reminder.time}, Language: {reminder.language}")
...
def home():
    reminders = Reminder.query.filter_by(
        user_id=current_user.id).order_by(Reminder.time).limit(5).all()
...
def submit_medication_response():
    ...
    reminder = Reminder.query.filter_by(
        user_id=current_user.id).order_by(Reminder.time.desc()).first()
    if reminder:
...
def skip_login():
    ...
    if not demo_user:
        demo_user = User(username="guest", password="guest123",
                         phone="+910000000000")
